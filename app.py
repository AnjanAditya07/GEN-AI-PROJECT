...
PANEL = "#141A22"        # sidebar / cards
PANEL_2 = "#1B2330"      # elevated surfaces
LINE = "#262F3B"         # hairline borders
TEXT = "#F2F4F7"         # primary text
MUTED = "#AEB6C4"        # secondary text (kept light enough to stay readable on near-black)
BRASS = "#E3A857"        # primary accent (data / action)
TEAL = "#5FBFAE"         # secondary accent (success / numeric)
RUST = "#D9705F"         # error accent

# Register a chart template that matches the app so Plotly figures
# don't look like a foreign, default-styled element dropped on the page.
...
# Chart-type picker options: "Auto" lets the AI decide (default); any other
# choice overrides it with a user-picked chart type, auto-fitted to the data.
...
def _is_generic_chart_followup(question: str, intent: dict) -> bool:
    ...
    # These words make the request about a new/specific slice of the data rather
    # than merely changing the presentation of the immediately previous result.
    ...
    # A generic chart request can be phrased many ways. Do not require a
    # particular verb: "a chart for better visualization" and "line chart of
    # this" should both work. The chart detector already established that this
    # turn explicitly asks for a visualization.
    return True
...
def render_assistant_result(msg: dict, idx: int):
    ...
    # Be backward-compatible with turns created by older app code that stored a
    # chart_spec but accidentally omitted the chart_requested flag.
    ...
    # ---- "only/just a chart" -> render nothing else ----
    if msg.get("chart_only") and df is not None:
        fig = None
        # Try the explicitly requested type first, then the AI's pick, then
        # fall back through every chart type so "only chart" still produces
        # *something* whenever the data can support any chart at all.
    ...
    # ---- Default: the plain-language summary, always shown ----
    st.markdown(_protect_causation_language(msg["content"]))

    # ---- SQL panel only when the user asked about the query ----
    # (also shown if the query itself failed, regardless of intent, since
    # that's diagnostic information rather than a normal answer)
    ...
    # Charts are explicit opt-in. Analytical questions show the evidence table
    # without a chart unless the user explicitly asks for a chart/graph/visualization.
    ...
    # Pre-select a specific chart type in the picker if the user named one.
    ...
    if pick == "Auto (AI decides)":
        # First use the deterministic business-aware spec. If that spec does not
        # fit (for example, an unusual SQL result shape), fall back to sensible
        # chart types automatically. A chart request should actually render a
        # chart whenever the result contains plottable data.
...
if question:
    ...
    # Figure out what this turn should actually show: the SQL panel only if
    # asked for, only-the-chart if asked for, and/or a specific chart type.
    # This never changes how the SQL is generated/run — only what's displayed.
    ...
    with st.chat_message("assistant", avatar=ASSISTANT_AVATAR):
        with st.status("Working on it…", expanded=False) as status:
            ...
            # A chart request can be a follow-up to the immediately preceding data result
            # (e.g. "display chart visual", "show a chart", "visualize it"). In that case,
            # reuse the previous result instead of asking the LLM to generate a new SQL query.
            # This is both faster and, importantly, prevents generic chart follow-ups from
            # becoming empty SELECTs because the follow-up itself contains no data metric.
            ...
            if generic_chart_followup and previous_data_msg is not None:
                ...
                # Preserve the analytical intent of the previous result so a
                # generic follow-up like "display chart visual" uses the right
                # visualization (e.g. line for a previous trend, bar for ranking).
            ...
            # Route conversational questions directly to natural language.
            # SQL is generated only when the question actually requires dataset analysis.
                ...
                if not sql:
                    ...
                    try:
                        ...
                        # One automatic repair pass prevents common DuckDB binder errors
                        # (especially GROUP BY + scalar benchmark CTE references) from
        ...
        # Render the final assistant turn outside the status container so it
        # persists in the normal chat flow — same helper the history loop uses,
        # so the manual chart-type picker works on it immediately too.
